      ...
      <main className="p-4 max-w-md mx-auto">
        <div className="grid grid-cols-2 gap-3 mb-4">
          <div className="bg-green-50 p-4 rounded-xl border border-green-200">
            ...
            <p className="text-2xl font-bold text-green-700">
              GHS {stats?.total_cost_savings_ghs?.toLocaleString() || 0}
            </p>
          </div>
          <div className="bg-purple-50 p-4 rounded-xl border border-purple-200">
            <p className="text-sm text-gray-600">🩺 DALYs</p>
            <p className="text-2xl font-bold text-purple-700">
              {stats?.total_dalys_averted?.toFixed(1) || 0}
            </p>
          </div>
          <div className="bg-blue-50 p-4 rounded-xl border border-blue-200">
            <p className="text-sm text-gray-600">⚠️ High Risk</p>
          ...
          <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-200 mb-4">
            ...
            <p className="text-xs text-gray-500 mt-1">
              {stats.average_icer_usd_per_daly < 2200 
                ? '✅ Cost-effective (below GDP per capita)' 
        ...
        {stats?.facility_breakdown && stats.facility_breakdown.length > 0 && (
          <div className="bg-white p-4 rounded-xl border border-gray-200">
            <h3 className="font-medium text-gray-700 mb-2">🏥 Facility Breakdown</h3>
            {stats.facility_breakdown.map((facility: any, i: number) => (
              <div key={i} className="flex justify-between border-b py-2 text-sm">
                ...
                <span className="font-medium text-green-600">
                  GHS {facility.cost_savings.toLocaleString()}
                </span>
              </div>
            ))}
          </div>
