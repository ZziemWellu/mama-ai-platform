  ...
  const handleAssess = async () => {
    ...
    try {
      const res = await fetch(
        'https://mama-ai-api.onrender.com/api/v1/assessments/assess',
        {
          method: 'POST',
              ...
              <>
                <div className="flex justify-between">
                  <div>
                    ...
                    <p className="text-2xl font-bold">{result.risk_level || 'UNKNOWN'}</p>
                  </div>
                ...
                {result.primary_condition && result.primary_condition !== 'NORMAL' && (
                  <div className="mt-2 p-2 bg-white bg-opacity-20 rounded">
                    <p className="font-semibold">{result.primary_condition}</p>
                ...
                {result.referral_options && result.referral_options.length > 0 && (
                  <div className="mt-2">
                    <p className="font-semibold text-sm">Nearby Facilities:</p>
                    {result.referral_options.map((facility: any, i: number) => (
                      <div key={i} className="bg-white bg-opacity-10 p-2 rounded mt-1 text-sm">
                        <p className="font-medium">{facility.facility_name}</p>
                        <p>📍 {facility.distance_km}km · {facility.estimated_travel_minutes}min</p>
                        <p>📞 {facility.phone}</p>
                      </div>
                    ))}
                  </div>
